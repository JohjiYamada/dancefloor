...
public class PropertyUtils {
	...
	public static String camelize(String s) {
		...
		s = s.toLowerCase();
		String[] array = PropertyUtils.split(s, "_");
		if (array.length == 1) {
			return PropertyUtils.capitalize(s);
		}
		StringBuffer buf = new StringBuffer(40);
		for (int i = 0; i < array.length; ++i) {
			buf.append(PropertyUtils.capitalize(array[i]));
		}
		return buf.toString();
	}
	...
	public static String camelizeLowerCase(String s) {
		if (s == null || s.length() == 0) {
			return "";
		}

		return camelize(s).substring(0, 1).toLowerCase() + camelize(s).substring(1);
	}
